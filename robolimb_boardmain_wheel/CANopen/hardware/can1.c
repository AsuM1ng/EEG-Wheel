...
void CAN1_RX0_IRQHandler(void)
{
	...
	{
		// Parse statusword (0x6041)
		// EPOS4 returns 0x43 (4-byte) or 0x4B (2-byte expedited) - accept both
		if ((rxm.data[0] == 0x43 || rxm.data[0] == 0x4B) &&
			rxm.data[1] == 0x41 && rxm.data[2] == 0x60 && rxm.data[3] == 0x00) {
			UNS8 node_id = rxm.cob_id - 0x580;
			if (node_id >= 1 && node_id <= 4) {
				// statusword: data[4]|data[5] (little-endian)
				g_statusword[node_id] = rxm.data[4] | (rxm.data[5] << 8);
			}
		}
		if (rxm.cob_id >= 0x581 && rxm.cob_id <= 0x584) {
			printf("SDO resp COB-ID=0x%03X cmd=0x%02X idx=0x%02X%02X sw=0x%04X\r\n",
					rxm.cob_id, rxm.data[0], rxm.data[2], rxm.data[1],
					rxm.data[4] | (rxm.data[5] << 8));
		}
