...
#define SEND_BUF_SIZE 64	//发送数据长度,最好等于sizeof(TEXT_TO_SEND)+2的整数倍.

u8 SendBuff[SEND_BUF_SIZE];	//发送数据缓冲区

const u8 TEXT_TO_SEND[]={"DevEBox大越电子 STM32F4 DMA 串口实验"};	 
...
{ 
  ...
  Gui_DrawFont_GBK16(0,32,BLUE,GRAY0,"  嵌入式开发网   ");	 
	...
	Gui_DrawFont_GBK16(0,64,BLUE,GRAY0,"KEY1:Start");	 
		 ...
		 while(1)
		    {
				...
				if(DMA_GetFlagStatus(DMA2_Stream7,DMA_FLAG_TCIF7)!=RESET)//等待DMA2_Steam7传输完成
					...
					sprintf((char*)tbuf,"%04.2f",pro);		
					
					Gui_DrawFont_GBK16(0,96,BLUE,GRAY0,tbuf) ;     //显示百分号     
	
		    }			    
				sprintf((char*)tbuf,"  100");		
